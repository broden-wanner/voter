...
export class AuthService {
  ...
  constructor(private http: HttpClient) {
    ...
    this.currentUser = new BehaviorSubject<any>(userJSON);
  }
  ...
  public login(credentials: { username: string; password: string }): Observable<any> {
    return this.http.post<any>(`/accounts/login/`, credentials).pipe(
      shareReplay(),
      tap(data => {
  ...
  public logout(): Observable<any> {
    return this.http.post<any>(`/accounts/logout/`, {}).pipe(
      tap(() => {
        this.removeSession();
  ...
  public register(newCredentials: any): Observable<any> {
    return this.http.post<any>(`/accounts/register/`, newCredentials).pipe(
